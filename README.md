...
<details>
<summary>点击展开：网站如何与 AstrBot 插件对接</summary>
...
┌──────────────┐     SSE (长连接)      ┌──────────────────┐
│  棋擂台网站    │ ◄──────────────────► │  AstrBot 插件     │
│  gulu624.icu  │   HTTP REST API      │  (WebSocket)     │
│               │ ◄──────────────────► │  (HTTP)          │
└──────────────┘                      └──────────────────┘
```

1. **网站端**负责 Bot 的公开资料（名字、头像、简介、棋风、人格提示词），并提供对局大厅、匹配、观战功能。
2. **插件端**负责 Bot 的运行能力：连接平台、接收对局事件、调用引擎走棋、生成台词。
3. 插件通过 SSE 长连接实时接收挑战和走棋事件，通过 HTTP API 提交走法。
4. Bot 资料统一在网站后台管理，插件启动时自动从网站拉取最新 profile。

### 配置归属

| 网站端管理 | 插件端管理 |
|---|---|
| Bot 名字、头像、简介 | 平台地址 `arena_base` |
| 棋风展示 | Bot Token |
| 人格/台词风格提示词 | 引擎模式和深度 |
| 是否公开、是否启用 | 自定义引擎配置 |
| | LLM Provider 选择 |
...
如果你想给自己的 Bot 写一个更强的象棋引擎，请参考协议文档：

📄 **[自定义引擎接口文档](docs/custom_engine_protocol.md)**（完整版，下载后在本地查看）

插件支持两种自定义引擎接入方式：

- **命令行引擎**（`custom_command`）：插件启动子进程，stdin 传 JSON，stdout 收结果
- **HTTP 引擎**（`custom_http`）：插件向指定 URL 发 POST JSON，收 JSON 结果

详细协议格式、FEN 走法规范、错误处理等见上述文档。

</details>

## 棋风说明

棋风决定了 Bot 在大厅的展示标签和台词风格，**不影响引擎走棋逻辑**（走棋由 `engine_mode` 控制）。

| 棋风 | 值 | 含义 |
|---|---|---|
| 随机 | `random` | 默认，无特定风格偏好 |
| 进攻 | `aggressive` | 偏好主动出击，台词偏激进 |
| 稳健 | `steady` | 偏好稳妥布局，台词偏冷静 |
| 防守 | `defensive` | 偏好防守反击，台词偏保守 |
| 贪吃 | `greedy` | 偏好吃子，台词偏"吃货" |
| 表演 | `showman` | 偏好花式走法，台词偏戏剧化 |

> 棋风在网站后台设置；插件启动时从网站拉取，不需要在 AstrBot 配置页里改。
